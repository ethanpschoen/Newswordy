...
const CompareAssociate: React.FC = () => {
  ...
  const handleFindCount = async () => {
    if (
      findingCount ||
      !selectedWord.trim() ||
      (wordCountGroupA !== undefined && wordCountGroupA !== null) ||
      (wordCountGroupB !== undefined && wordCountGroupB !== null)
    ) {
      return
    }
    ...
    try {
      ...
      const { data: dataGroupB, error: errorGroupB } = await gameAPI.getWordCount(
        selectedTimePeriod,
        groupB,
        selectedWord,
        new Date(),
    ...
    <Container maxWidth="xl">
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
              ...
              <Chip label={`${groupA.length} source${groupA.length !== 1 ? 's' : ''}`} color="primary" size="small" />
            </Box>
            <Divider sx={{ mb: 2 }} />
            {groupA.length === 0 ? (
              <Box
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
              ...
              <Chip label={`${groupB.length} source${groupB.length !== 1 ? 's' : ''}`} color="secondary" size="small" />
            </Box>
            <Divider sx={{ mb: 2 }} />
            {groupB.length === 0 ? (
              <Box
      ...
      <Paper elevation={2} sx={{ p: 3, mb: 4 }}>
        ...
        {availableSources.length === 0 ? (
          <Alert severity="success">All sources have been assigned to groups!</Alert>
        ) : (
          <Grid container spacing={2}>
            {availableSources.map(source => (
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={source}>
                <AvailableSourceCard
                  source={source}
                  handleAddToGroupA={handleAddToGroupA}
                  handleAddToGroupB={handleAddToGroupB}
                />
              </Grid>
            ))}
          </Grid>
      ...
      <Card sx={{ mb: 4, boxShadow: 3 }}>
        <CardContent>
          ...
          {wordCountGroupA !== null &&
            wordCountGroupA !== undefined &&
            wordCountGroupB !== null &&
            wordCountGroupB !== undefined && (
              <Alert
              ...
              >
                ...
                >
                  <Typography variant="body1" sx={{ fontWeight: 600 }}>
                    <strong>{wordCountGroupA.toLocaleString()}</strong> instance{wordCountGroupA !== 1 ? 's' : ''} found
                    in Group A and <strong>{wordCountGroupB.toLocaleString()}</strong> instance
                    {wordCountGroupB !== 1 ? 's' : ''} found in Group B
                  </Typography>
                  {wordCountGroupA > 0 && wordCountGroupB > 0 && (
                    <Chip label="Ready to play" color="success" size="small" sx={{ fontWeight: 600 }} />
                  )}
                </Box>
              </Alert>
            )}
