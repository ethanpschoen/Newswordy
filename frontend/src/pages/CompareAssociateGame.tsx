...
import { CompareAssociateGameState, Guess, ScoreboardEntry, ComparativeScoreboardEntry, ComparativeGroup } from '../types'
import { 
  Box,
  Button,
  Typography,
...
const CompareAssociateGame: React.FC = () => {
  ...
  const { compareAssociateId: gameId } = useParams<{ compareAssociateId: string }>()

  const navigate = useNavigate()

  // Game state variables
  const [gameState, setGameState] = useState<CompareAssociateGameState | null>(null)
  ...
  }, [scoreboardGroupA, showScoreboard, gameState?.compare_associate_guesses])
  ...
  const loadGame = async () => {
    try {
      ...
      // Get game state from database
			const gameResponse = await gameAPI.getComparativeAssociatedGameState(gameId || '')
			...
			const scoreboardResponse = await gameAPI.getComparativeAssociatedScoreboard(game.time_period, game.sources_group_a, game.sources_group_b, game.word, game.scoreboard_size, new Date(game.created_at))
  ...
  const handleSubmitGuess = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (gameState!.compare_associate_guesses.some(guess => guess.word === guessWord)) {
      ...
      const updatedGuesses = [...gameState?.compare_associate_guesses || [], newGuess]

      const updatedGameState = {
        ...gameState!,
        score: updatedScore,
        compare_associate_guesses: updatedGuesses,
      ...
      await gameAPI.submitComparativeAssociatedGuess(newGuess)

      let { compare_associate_guesses: _, ...updatedGame } = updatedGameState
      ...
      if (updatedGame.is_completed) {
        ...
        if (isAuthenticated) {
          ...
          await userAPI.updateUser(newStats, user?.sub || '')
			...
			await gameAPI.updateComparativeAssociatedGameState(updatedGame, updatedGame.id)
    } catch (error: any) {
  ...
  const groupAccentColor = theme.palette.primary.main
    ...
    return (
      <Container maxWidth="xl" sx={{ pb: 18 }}>
        <Grid container spacing={2}>
          ...
          {/* Recent Guesses */}
          {gameState.compare_associate_guesses.length > 0 && (
            <Grid size={{ xs: 12 }} sx={{ order: 6 }}>
              <GuessList guesses={gameState.compare_associate_guesses} />
            </Grid>
          )}
          ...
          <Stack spacing={2} sx={{ 
            ...
            <GameStats guessedWords={gameState.guessed_words_group_a.concat(gameState.guessed_words_group_b)} score={gameState.score} remainingGuesses={gameState.remaining_guesses} />
					...
					{/* Group A Article Info */}
						<ArticleInfo selectedWordData={selectedWordDataGroupA} currentPage={currentPageGroupA} articlesPerPage={articlesPerPage} setCurrentPage={setCurrentPageGroupA} closeArticlePanel={closeArticlePanel} scoreboardHeight={scoreboardHeight} groupLabel="Source Group A" groupAccentColor={groupAccentColor} />
          </Stack>
        </Grid>
