...
expense of context and nuance.
This contributes to \emph{news fatigue}, disengagement, and selective exposure,
where people either tune out entirely or restrict themselves to a narrow set of
ideologically aligned sources.\cite{news-polarization-cable-tv,news-avoidance-fatigue}
...
actually happening in the world.\cite{misinformation-polarization}
...
and algorithmically curated social feeds.\cite{selective-exposure}
...
Newswordy is a web-based word-guessing game that operationalizes this idea.
...
The game builds on the success of daily casual games such as Wordle and
crosswords, which have been shown to support regular engagement and incidental
learning.\cite{serious-games-learning,gamification-engagement}
...
ideological polarization.\cite{news-polarization-cable-tv}
...
outrage-inducing, or identity-affirming content.\cite{misinformation-polarization}
...
mental health.\cite{news-avoidance-fatigue}
...
\subsection{Headlines, Word Frequencies, and Co-occurrence}

From a computational perspective, Newswordy treats headlines as compact
summaries of what newsrooms consider most important.
Headlines are not neutral data --- they reflect editorial choices and
institutional biases --- but they provide a tractable lens on the \emph{agenda}
of different outlets.

By tokenizing headlines into words, filtering out stop words, and aggregating
counts over time, we obtain simple but interpretable statistics:
which words are most prominent overall, how their frequencies change over time,
and how those distributions differ between groups of sources.
Similarly, co-occurrence statistics --- which words commonly appear in
headlines alongside a given ``anchor'' word --- offer a window into how a topic
is framed (for example, whether ``immigration'' co-occurs more with ``border''
or ``workers'').

These techniques are intentionally lightweight compared to more complex
topic modeling or representation-learning approaches.
The goal is not to infer hidden ideological dimensions, but to expose
patterns that non-experts can understand and reason about:
``these are the words showing up in headlines right now, and here is how they
change when I switch sources or time ranges.''

\subsection{Scope of Technical Discussion}

Newswordy is implemented using a web frontend, a PostgreSQL database, and a
Python-based scraping pipeline.
However, in keeping with the rubric, the remainder of this paper focuses on the
problem formulation, design choices, and evaluation rather than the specifics
of languages or libraries.
High-level architecture and replication details are provided in later sections
and in the public GitHub repository.
...
judgments.\cite{media-bias-charts}
Other tools aggregate headlines into dashboards or topic maps, allowing users
to see how different outlets are covering particular stories.
...
Mainstream outlets frequently offer weekly ``news quiz'' articles that test
readers on the details of recent stories.
...
support vocabulary and pattern learning.\cite{wordle-article}
...
asks players to make educated guesses about words under light time pressure.
...
such as education, training, or civic engagement.\cite{serious-games-learning}
...
between game mechanics and learning goals is crucial.\cite{gamification-engagement}

In designing Newswordy, the primary goal was not to maximize time-on-site,
but to encourage brief, repeated interactions that leave players with a clearer
mental model of what the news is about.
The use of points and streaks is intended to support habit formation, while the
ability to click into underlying articles and explore advanced comparison modes
supports deeper reflection for users who want it.
...
\begin{itemize}
    ...
    groups of outlets talk about the same period or anchor word.
    \item \textbf{Transparency:} always allow players to see the underlying
    articles behind aggregate statistics.
...
outlets and a range of ideological perspectives.
...
When a user starts a game, they configure a time window (for example, the past
day or week) and a set of sources or source groups.
The backend then:
\begin{enumerate}
...
computes separate frequency counts for each group and then derives a
comparison score (for example, the difference in normalized frequencies).
...
they can run close to the data and respond in real time as users adjust
settings.
...
\begin{description}
    ...
    This highlights differences in framing and agenda between outlets.
    ...
    players explore how different outlets talk about the same topic.
\end{description}
...
Hints are available to keep the experience approachable, including partial
reveals of word shapes and lists of most common words.
...
User accounts and profiles allow players to revisit past games, see historical
scores, and track their engagement over time, supporting habit formation.

\section{Evaluation Metrics}

Evaluating Newswordy requires assessing both its educational value and its
ability to engage users without overwhelming them.
...
tests administered days later, or large-scale telemetry on repeated play.
...
burden of contemporary news consumption.\cite{news-avoidance-fatigue}
...
Several noted that the game was harder than expected because many frequent
words were descriptive or thematic (for example, issue or place names) rather
than the proper nouns they initially guessed.
...
often in left-leaning versus right-leaning outlets, or how references to
religious figures clustered in particular source groups.
...
how certain topics (such as immigration or climate) are consistently linked to
particular frames or co-occurring issues.
...
success rather than exploration.\cite{gamification-engagement}
...
Others suggested automatically opening the article list when a word is guessed
and clearly indicating which word's articles are being shown.
There were also requests for more in-context explanation of certain advanced
features and for small enhancements like displaying the number of letters in
each word.
...
of college students, it is not clear how well the game would generalize to
other demographics.
...
Overall, the evaluation suggests that Newswordy can function as a useful
on-ramp to news awareness and comparative media analysis, but that careful
design choices are required to avoid merely repackaging existing habits in a
more entertaining form.
...
``mainstream'' and whose are peripheral.\cite{media-bias-charts}
...
gamification in civic and political contexts.\cite{serious-games-learning}
...
run Newswordy using the public GitHub repository.
...
At a high level, Newswordy is organized into three main components: the web
frontend, the news scraping and processing pipeline, and the database layer
that stores articles and supports word-frequency queries.
...
time, while additional tables or views support aggregated word counts and
